...
def order_insert(graph, u, v):
    ...
    while c_node is not k_order[k]._header:
        ...
        if (graph.nodes[c_node.element]['remaining_degree'] + graph.nodes[c_node.element]['candidate_degree'] > k):
            ...
            for neighbor in graph[c_node.element]:
                if (core[neighbor] == k and graph.nodes[c_node.element]['micd'] != k and
                        (treap_order[k].rank(c_node.element, treap_map) < treap_order[k].rank(neighbor, treap_map))):
                    graph.nodes[neighbor]['candidate_degree'] += 1
                    # Since the neighbors canidate_degree is greater than 0 it gets pushed on
                    # the heap B.
    ...
    for node in reversed(candidates):  # reversed() to maintain the order the candidates had in k_order[k].
        for neighbor in graph[node.element]:
            if core[neighbor] == k:
                graph.nodes[node.element]['micd'] += 1
...
graph = file_to_graph('C:\\Users\\anika\\Desktop\\git-project\\BA-Project\\Testing\\Graphs\\CA-AstroPh.txt')
