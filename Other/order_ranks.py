...
def rank_init(k_order):
    for i in range(len(k_order)):
        rank = 0
        for node in k_order[i]:
            ranks[node] = rank
            rank += 1
...
def order_insert(graph, u, v):
    ...
    if core[u] < core[v]:
        ...
        graph.nodes[v]['mcd'] += 1
        graph.nodes[u]['mcd'] += 1
        if ranks[dbll_map[u]] < ranks[dbll_map[v]]:
            root = u
            graph.nodes[u]['remaining_degree'] += 1
        else:
            root = v
    ...
    if graph.nodes[root]['remaining_degree'] <= k:
        return
    # heapq.heappush(B, (ranks[dbll_map[root]], root))
    c_node = k_order[k]._trailer.next
    # init_vertex = None
    if dbll_map[root].prev is not k_order[k]._trailer:
        # init_vertex = dbll_map[root].prev
        k_order[k].dbll_block_append(c_node, dbll_map[root].prev, new_ok, ranks[dbll_map[root]] - 0)
        c_node = k_order[k]._trailer.next
    next_vertex = None
    while c_node is not k_order[k]._header:
        ...
        if (graph.nodes[c_node.element]['remaining_degree'] + graph.nodes[c_node.element]['candidate_degree'] > k):
            ...
            if next_vertex is c_node:
                if new_ok._header.prev is None:
                    obs_6[c_node] = (list(obs_6.keys())[-1], new_ok._trailer)
                else:
                    obs_6[c_node] = (list(obs_6.keys())[-1], new_ok._header.prev)
            elif new_ok._header.prev is None:
                obs_6[c_node] = new_ok._trailer
            else:
                obs_6[c_node] = new_ok._header.prev
            next_vertex = c_node.next
            candidates[k_order[k].dbll_remove(c_node)] = None
            for neighbor in graph[c_node.element]:
                if core[neighbor] == k and ranks[c_node] < ranks[dbll_map[neighbor]]:
                    graph.nodes[neighbor]['candidate_degree'] += 1
                    # Since the neighbors canidate_degee is greater than 0 it gets pushed on
                    # the heap B.
                    heapq.heappush(B, (ranks[dbll_map[neighbor]], neighbor))
            c_node = k_order[k]._trailer.next
        # Branch 2
            ...
            try:
                ...
                k_order[k].dbll_block_append(c_node, dbll_map[jump_node].prev, new_ok, ranks[dbll_map[jump_node]] - ranks[c_node])
                c_node = k_order[k]._trailer.next
            except IndexError:
    ...
    for node in obs_6:
        if node not in candidates:
            try:
                if obs_6[node][0] in candidates:
                    if obs_6[node][1] is new_ok._trailer:
                        ranks[node] = ranks[new_ok._trailer.next]-0.001
                        new_ok.dbll_obs_6(obs_6[node][1], node)
                    else:
                        new_ok.dbll_obs_6(obs_6[node][1], node)
                        ranks[node] = ranks[node.prev] + 0.001
                else:
                    new_ok.dbll_obs_6(obs_6[node][0], node)
                    ranks[node] = ranks[node.prev] + 0.001
            except TypeError:
                if obs_6[node] is new_ok._trailer:
                    # global rank_gap
                    # ranks[node] = (-0.001 - rank_gap)
                    # rank_gap += 0.001
                    ranks[node] = ranks[new_ok._trailer.next]-0.001
                    new_ok.dbll_obs_6(obs_6[node], node)
                else:
                    new_ok.dbll_obs_6(obs_6[node], node)
                    ranks[node] = ranks[node.prev] + 0.001
        # ranks[c_node] = ranks[c_node.prev] + 1
    for node in reversed(candidates):  # reversed() to maintain the order the candidates had in k_order[k].
        ...
        ranks[node] = ranks[node.next]-1
        # for neighbor in graph[node.element]:
        #    if core[neighbor] == k+1 and dbll_map[neighbor] not in candidates:
        #        graph.nodes[neighbor]['remaining_degree'] += 1
        #        graph.nodes[node.element]['remaining_degree'] -= 1
        # treap_order[k].treap_prepend(treap_order[k+1], treap_map[node.element])
    k_order[k] = new_ok
    # if init_vertex is None:
    #     init_vertex = new_ok._trailer.next  # Problems wenn all vertices change (highly unlikely)
    #    ranks[init_vertex] = 0
    # while init_vertex is not new_ok._header.prev:
    #     ranks[init_vertex.next] = ranks[init_vertex] + 1
    #     init_vertex = init_vertex.next
...
def remove_candidates(graph, new_ok, b_set, node, k, obs_6, candidates):
    ...
    while not q.empty():
        ...
        # new_ok.dbll_obs_6(obs_6[c_node], c_node)
        ...
        for neighbor in k_neighbor:
            ...
            if ranks[node] < ranks[dbll_map[neighbor]]:
                graph.nodes[neighbor]['candidate_degree'] -= 1
                if graph.nodes[neighbor]['candidate_degree'] == 0:
                    b_set.add(neighbor)  # Lazy removal of 'neighbor' from B.
            elif neighbor_node in candidates and (ranks[c_node] < ranks[dbll_map[neighbor]]):
                graph.nodes[neighbor]['candidate_degree'] -= 1
