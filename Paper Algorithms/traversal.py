...
def traversal_insert(graph, n, u, v):
    ...
    candidates = set()  # Stores all nodes that might their core number changed.
    s.append(root)
    ...
    while s:
        node = s.pop()
        reset_attr.add(node)
        if graph.nodes[node]['cd'] > k:  # Node has enough suitable neighbors to increase its core number.
            candidates.add(node)
            for neighbor in graph[node]:
